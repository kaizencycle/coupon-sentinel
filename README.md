...
**Not yet built** (needs real accounts/credentials + further sessions, roughly Milestones 5-7 of the phase-1 plan):
Mixpanel/Sentry, and production deployment of this new surface. The mock-data optimizer (`/api/optimize`, `/api/deals`, etc.) and its original frontend views remain fully functional and are not gated by auth.
